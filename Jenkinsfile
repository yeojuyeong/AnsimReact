pipeline {
    ...
    stages {
        ...
        stage("Docker Image Build") {
           steps {
               sh "docker build -t yjy8606/apache2_ansimreact:${BUILD_NUMBER} ./docker/apache2/"
           }
        }
        stage("Docker Image Push") {
           steps {
               sh "docker push yjy8606/apache2_ansimreact:${BUILD_NUMBER}"
           } 
        }
        stage("Docker Image Clean up") {
           steps {
               sh "docker image rm yjy8606/apache2_ansimreact:${BUILD_NUMBER}" 
           }
        }
