...
const BoardModify = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            //게시물 상세 가져오기
            const response = await axios.get(`http://localhost:8080/restapi/modify?seqno=${seqno}&&page=${page}&keyword=${keyword}`);
                        ...
                        {/*        <input type="hidden" id="keyword" name="keyword" th:value="${keyword}">*/}
                                    <div className="top">
                                        ...
                                        <div className="bigRight">
                                            <input type="text" id="title" className="sub-detail-title" name="title" value={title} ref={titleRef} onChange={(e) => setTitle(e.target.value)} />
                                                <div className="sub-detail">
                                                    <input type="text" id="departure" className="left" name="departure" value={departure} ref={departureRef} onChange={(e) => setDeparture(e.target.value)} />
                                                        <input type="text" id="destination" className="right" name="destination" value={destination} ref={destinationRef} onChange={(e) => setDestination(e.target.value)} />
                                                </div>
                                                <div className="sub-detail">
                                                    <div className="left">
                                                        <div className="date-time-selector">
                                                            <label htmlFor="date">날짜:</label>
                                                            <input name="date" type="date" id="date" value={date} onChange={(e) => setDate(e.target.value)} />
                                                ...
                                                <div className="sub-detail">
                                                    ...
                                                    <div className="right">
                                                        대화 :
                                                        <label><input type="radio" name="sound" value="OPT_SOUND_1" checked={sound === '조용히'|| sound === 'OPT_SOUND_1'} onChange={(e)=>setSound(e.target.value)} />조용히</label>
                                                        <label><input type="radio" name="sound" value="OPT_SOUND_2" checked={sound === '상관없음'|| sound === 'OPT_SOUND_2'} onChange={(e)=>setSound(e.target.value)} />상관없음</label>
                                                    </div>
                                                </div>
                                        </div>
                                    </div>
